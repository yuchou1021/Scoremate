...
> **状态：v0.4.7 已发布** —— 功能在 MuseScore 4.7.4 实测通过；v0.4.6 为 UI 布局修复（内容自适应 + 可滚动），v0.4.7 为写回增强（执行建议按钮 + 压缩音域整句移动），需在 MuseScore 中确认效果。
...
- **「执行以上建议」按钮**：分析生成指令后，一键把已支持的指令写回乐谱（移调/压缩音域/去重复八度），全部 Ctrl+Z 可撤销；未支持的规则（删装饰音/拆和弦/简化节奏）明确提示 v0.2 接入
